...
(deb3 <- with(subset(pres_debates2012, time=="time 3"),
    polarity(dialogue, person)))

## Set up the network version
bg_black <- Animate(deb3, neutral="white", current.speaker.color="grey70")
...
deb3_bar <- Animate(deb3, as.network=FALSE)
...
FUN2 <- function(follow=FALSE) {

    Title <- "Animated Polarity: 2012 Presidential Debate 3"
